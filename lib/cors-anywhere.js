...
const ServerConfigurator = require('../server-config');
...
const manipulatorConfig = {
  allowedMethodsConcatenated: null,
...
function withCORS(headers, request) {
  ...
  if (manipulatorConfig.wildcardOrigin) {
    headers['access-control-allow-origin'] = '*';
  } else {
    if ('origin' in request.headers) {
      headers['access-control-allow-origin'] = request.headers['origin'];
    } else {
      logger.warn(`Requesting ressource without origin set. Set dummy value`);
      headers['access-control-allow-origin'] = 'NO_ORIGIN_SET!!!';
    }
    if (headers['vary']) {
      headers['vary'] += ',Origin';
    } else {
  ...
  if (manipulatorConfig.corsMaxAge) {
    headers['access-control-max-age'] = manipulatorConfig.corsMaxAge;
  }

  var allowedMethods = manipulatorConfig.allowedMethodsConcatenated || request.headers['access-control-request-method'];
  if (allowedMethods) {
          ...
          return proxyReqOn.call(this, 'response', function(proxyRes) {
            if (onProxyResponse(proxy, proxyReq, proxyRes, req, res)) {
              try {
                ...
                logger.error('Error proxy request: ' + err.toString());
...
function onProxyResponse(proxy, proxyReq, proxyRes, req, res) {
  ...
  // Handle redirects
  if (manipulatorConfig.handleRedirects && (statusCode === 301 || statusCode === 302 || statusCode === 303 || statusCode === 307 || statusCode === 308)) {
    ...
    if (locationHeader) {
      ...
      if (statusCode === 301 || statusCode === 302 || statusCode === 303) {
        ...
        if (requestState.redirectCount_ <= manipulatorConfig.maxRedirects) {
  ...
  manipulatorConfig.responseHeadersToRemove.forEach((key) => delete proxyRes.headers[key]);

  // Override response headers with those specifed by the user
  Object.keys(manipulatorConfig.responseHeadersToSet).forEach(function (key) {
    proxyRes.headers[key] = manipulatorConfig.responseHeadersToSet[key];
  });

  proxyRes.headers['x-final-url'] = requestState.location.href;
  withCORS(proxyRes.headers, req);

  logger.debug('Response result headers: ' + JSON.stringify(proxyRes.headers));
...
/**
 * Initializes manipulator object which needs to be processed only once.
 * @param config Mostly der server configuration.
 */
function initGlobalManipulator(config) {
  manipulatorConfig.handleRedirects = config.handleRedirects;
  manipulatorConfig.responseHeadersToRemove = config.removeResponseHeader;
  manipulatorConfig.responseHeadersToSet = config.setResponseHeaders;
  manipulatorConfig.allowedMethodsConcatenated = config.allowedMethods.join(',');
  manipulatorConfig.maxRedirects = config.maxRedirects;
  manipulatorConfig.wildcardOrigin = config.wildcardOrigin;
  manipulatorConfig.corsMaxAge = config.corsMaxAge;
}
...
function getHandler(options, proxy) {
  const corsAnywhere = {
    ...
    setResponseHeaders: {},         // Override the response with these headers.
    wildcardOrigin: true,           // Send back a wildcard or reflect the incoming origin
    removeResponseHeader: [],
    corsMaxAge: null,                  // If set, an Access-Control-Max-Age header with this value (in seconds) will be added.
    handleRedirects: true,
  ...
  initGlobalManipulator(corsAnywhere);
  ...
  return function (req, res) {
    ...
    if (corsAnywhere.allowedMethods.length && corsAnywhere.allowedMethods.indexOf(req.method) === -1) {
      res.writeHead(405, 'Method not allowed', cors_headers);
      res.end('Method must be one of: ' + corsAnywhere.allowedMethods);
      return;
    }
...
exports.createServer = function createServer(options) {
  const defaultOptions = ServerConfigurator.getConfiguration();
  const mergedOptions = Object.assign({}, defaultOptions, options);
  const httpProxyOptions = Object.assign({}, mergedOptions.httpProxyOptions);

  const proxy = httpProxy.createServer(httpProxyOptions);
  const requestHandler = getHandler(mergedOptions, proxy);

  logger.debug('Options passed to proxy: ' + mergedOptions.toString());

  let server;

  if (mergedOptions.httpsOptions) {
    logger.debug('Created HTTPS Server');
    server = require('https').createServer(mergedOptions.httpsOptions, requestHandler);
  } else {
