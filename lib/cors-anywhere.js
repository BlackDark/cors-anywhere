...
let handleRedirects = true;
let responseHeadersToRemove = [];
...
function withCORS(headers, request) {
  ...
  if (request.corsOptions.wildcardOrigin) {
  ...
  var corsMaxAge = request.corsOptions.corsMaxAge;
  if (corsMaxAge) {
    headers['access-control-max-age'] = corsMaxAge;
  }

  var allowedMethods = request.corsOptions.allowedMethodsString || request.headers['access-control-request-method'];
  if (allowedMethods) {
...
function onProxyResponse(proxy, proxyReq, proxyRes, req, res) {
  ...
  // Handle redirects
  if (handleRedirects && (statusCode === 301 || statusCode === 302 || statusCode === 303 || statusCode === 307 || statusCode === 308)) {
  ...
  responseHeadersToRemove.forEach((key) => delete proxyRes.headers[key]);

  // Override response headers with those specifed by the user
  Object.keys(requestState.setResponseHeader).forEach(function (key) {
    proxyRes[key] = requestState.setResponseHeader[key];
  });
...
function getHandler(options, proxy) {
  ...
  handleRedirects = corsAnywhere.handleRedirects;
  responseHeadersToRemove = corsAnywhere.removeResponseHeader;
  ...
  return function (req, res) {
    // Set the options to apply when generating CORS headers on the request
    req.corsOptions = {
      allowedMethodsString: corsAnywhere.allowedMethodsString = corsAnywhere.allowedMethods.join(','),
      wildcardOrigin: corsAnywhere.wildcardOrigin,
    ...
    req.corsAnywhereRequestState = {
      ...
      setResponseHeader: corsAnywhere.setResponseHeaders,
    };
